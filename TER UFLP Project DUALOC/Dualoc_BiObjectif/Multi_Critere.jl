...
function diophentiennesToAffine(drt::Droite)
 return Droite(-(drt.a * denominator(drt.b)),denominator(drt.b),drt.ind)
end
...
function resolutionBiObjectif(c::Array{Array{Int64,2},1},f::Array{Array{Int64,1},1})
	...
	function recInitResolutionBiObjectif(c::Array{Droite,2},f::Array{Droite,1},MatBoolC::Array{Bool,2},VecteurDernierCout::Array{Droite,1},indiceColonne::Int64,deb::Rational{Int64},fin::Rational{Int64},BoolI::Array{Bool,1},noeud::Arbre)
		...
		vecDroite = lowerHull(VecteurDroiteSansInf(c[:,indiceColonne]),deb,fin,1//10)
        ...
        if (all(BoolI)!=true) #Condition d'arret si tous les client sont saturé ou si tous les coût associés au service sont saturés
            ...
            if (BoolI[indiceColonne]==false)
				...
				if(size(droiteSansInf)[1]>0)
					vecDroite = lowerHull(droiteSansInf,deb,fin,1//1000) # 1 argument correspond au droites != inf et pas
					...
					for i in 1:size(vecDroite)[1]
						...
						lowerHullVecFPLusDelta = lowerHull(vecFPLusDelta,tempDeb,tempFin,1//1000)
						...
						for j in 1:size(lowerHullVecFPLusDelta)[1]
							...
							noeudAct = Arbre(tempDebF,tempFinF,tempVecteurDernierCout,tempMatBoolC[:,indiceColonne],[])
							push!(noeud.fils,noeudAct)
							println("BoolIFin : ", tempBoolI)
							recResolutionBiObjectif(c,tempF,tempMatBoolC,tempV,tempVecteurDernierCout,mod(indiceColonne,size(c,2))+1, tempDebF, tempFinF,tempBoolI,noeudAct)
						end
					...
					#Calcul des différents deltas en fonction de f
					lowerHullVecFPLusDelta = lowerHull(servicesUtilise,tempDeb,tempFin,1//1000)
					...
					for j in 1:size(lowerHullVecFPLusDelta)[1]
						...
						noeudAct = Arbre(tempDebF,tempFinF,tempVecteurDernierCout,tempMatBoolC[:,indiceColonne],[])
						push!(noeud.fils,noeudAct)
						recResolutionBiObjectif(c,tempF,tempMatBoolC,tempV,tempVecteurDernierCout,mod(indiceColonne,size(c,2))+1, tempDeb, tempFin,tempBoolI,noeudAct)
					end
				end
			else
