...
function exemple()
    ...
    sol = dichotomy(d)
    x = [sol[i].z1 for i in 1:length(sol)]
	y = [sol[i].z2 for i in 1:length(sol)]
	return (x,y)
end

function exemple2()
    # Saisie des données de l'exemple des arbres du rapport
    f1::Vector{Int64} = [10,7,4]
    c1::Matrix{Int64} = [10 20 10 40;
                         50 10 10 10000;
                         70 30 90 60]
    f2::Vector{Int64} = [12,8,7]
    c2::Matrix{Int64} = [30 60 10000 40;
                         80 80 10 20;
                         10 10 70 10]
    d::donneesUFLP = donneesUFLP(3,4,f1,f2,c1,c2)
...
function exemple3()
    ...
    d::donneesUFLP = donneesUFLP(30,90,f[1],f[2],c[1],c[2])
...
function afficher1(sol::Tuple{Array{Float64,1},Array{Float64,1}})
	display(scatter(sol[1],sol[2],label="p"))
end

function afficher(sol::Array{solutionUFLP})
	x = [sol[i].z1 for i in 1:length(sol)]
	y = [sol[i].z2 for i in 1:length(sol)]
	print(x,y)
